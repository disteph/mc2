...
let solve
    ?dec_heur ?gc ?restarts ?dot_proof
    ?(pp_model=false) ?(check=false) ?time ?memory ?progress
    ~assumptions s : unit =
  let t1 = Sys.time() in
  let res = Solver.solve ?dec_heur ?gc ?restarts ?time ?memory ?progress s ~assumptions in
  let t2 = Sys.time () in
...
let process_stmt
    ?dec_heur ?gc ?restarts ?(pp_cnf=false) ?dot_proof ?pp_model ?check
  ...
  begin match stmt with
    ...
    | A.Exit ->
      ...
      raise Exit
    | A.CheckSat ->
      solve ?dec_heur ?gc ?restarts ?dot_proof ?check ?pp_model ?time ?memory ?progress
        solver ~assumptions:[];
      E.return()
    | A.TyDecl (id,n) ->
      decl_sort id n;
